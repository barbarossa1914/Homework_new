...
class Graph:
    ...
    def add_edge(self, u, v, string):
        self.graph[u].append((v, string))
        self.vertices.add(u)
        self.vertices.add(v)

    def dijkstra(self, start):
        visited = {vertex: False for vertex in self.vertices}
        dist = {vertex: 'z'*1000 for vertex in self.vertices}
        dist[start] = ''
        ...
        while True:
            ...
            for neighbor in self.graph[min_vertex]:
                new_dist = min(dist[min_vertex] + neighbor[1], dist[neighbor[0]])
                dist[neighbor[0]] = new_dist
        return dist

graph_1 = Graph()
graph_1.add_edge(1, 2, 'a')
graph_1.add_edge(2, 3, 'b')
graph_1.add_edge(1, 3, 'aa')
